...
impl<'a> ImageProcessor<'a> {
    pub fn new(final_resolution: (u32, u32), frames_dir: &'a Path, upscaled_frames_dir: &'a Path) -> Self {
        let thread_pool = rayon::ThreadPoolBuilder::new()
            // .num_threads(4)
            .build()
            .unwrap();
        ImageProcessor { final_resolution, frames_dir, upscaled_frames_dir, thread_pool }
    }
    ...
    pub fn upscale_frames(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let errors: Vec<String> = self.thread_pool.install(|| {
            entries.par_iter().filter_map(|entry| {
                self.upscale_and_save_image(entry, &pb)
            }).collect::<Vec<_>>()
        });
    ...
    fn upscale_and_save_image(&self, entry: &fs::DirEntry, pb: &ProgressBar) -> Option<String> {
        ...
        if frame_path.is_file() && frame_path.extension().unwrap_or_default() == "png" {
            if let Ok(image) = self.load_image(&frame_path) {
                ...
                let upscaled_path = self.upscaled_frames_dir.join(frame_path.file_name().unwrap());
                if upscaled_image.save(upscaled_path.clone()).is_err() {
                    Some(format!("Failed to save upscaled frame: {}", upscaled_path.display()))
                } else {
                    pb.inc(1);
                    None
                }
    ...
    fn bilinear_interpolation(&self, image: &ImageBuffer<Rgb<u8>, Vec<u8>>) -> ImageBuffer<Rgb<u8>, Vec<u8>> {
        ...
        for y in 0..new_height {
            for x in 0..new_width {
                ...
                let c10 = image.get_pixel((gxi + 1).min(width - 1), gyi);
                let c01 = image.get_pixel(gxi, (gyi + 1).min(height - 1));
                let c11 = image.get_pixel((gxi + 1).min(width - 1), (gyi + 1).min(height - 1));

                let weights = [
                    (1.0 - gx + gxi as f32) * (1.0 - gy + gyi as f32),
                ...
                let pixel = Rgb([
                    (c00.0[0] as f32 * weights[0] + c10.0[0] as f32 * weights[1] + c01.0[0] as f32 * weights[2] + c11.0[0] as f32 * weights[3]) as u8,
                    (c00.0[1] as f32 * weights[0] + c10.0[1] as f32 * weights[1] + c01.0[1] as f32 * weights[2] + c11.0[1] as f32 * weights[3]) as u8,
                    (c00.0[2] as f32 * weights[0] + c10.0[2] as f32 * weights[1] + c01.0[2] as f32 * weights[2] + c11.0[2] as f32 * weights[3]) as u8,
                ]);
